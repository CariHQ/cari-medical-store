...
import VendorDeliveryButtons from "./vendor/delivery-buttons";
import { VendorDeliveryStatusBadge } from "./vendor/delivery-status-badge";
   ...
   return (
      <Container className="flex flex-col gap-6 p-4 shadow-elevation-card-rest hover:shadow-elevation-card-hover w-full">
         <div className="flex justify-between gap-2 flex-wrap">
            ...
            {type === "driver" && (
               <DriverDeliveryStatusBadge delivery={delivery} />
            )}
            {type === "vendor" && (
               <VendorDeliveryStatusBadge delivery={delivery} />
            )}
         </div>
               ...
               <Table.Body>
                  {delivery.items?.map((item) => (
                     <Table.Row
                        ...
                        <Table.Cell className="text-wrap">
                           {item.title}
                        </Table.Cell>
         ...
         <div className="flex gap-4 justify-end">
            ...
            {type === "vendor" && <VendorDeliveryButtons delivery={delivery} />}
         </div>
      </Container>
   );
}
