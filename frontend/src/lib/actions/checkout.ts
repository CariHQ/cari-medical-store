...
export async function createDelivery(cartId: string, vendorId: string) {
   const delivery = await fetch(`${BACKEND_URL}/deliveries`, {
      method: "POST",
      headers: {
         "Content-Type": "application/json",
      },
      body: JSON.stringify({ cart_id: cartId, vendor_id: vendorId }),
   })
      .then((res) => res.json())
      .catch((error) => {
...
export async function placeOrder(prevState: any, data: FormData) {
   ...
   const vendorId = data.get("vendor-id")?.toString();
   ...
   try {
      ...
      const delivery = await createDelivery(cartId, vendorId);
